...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("SELECT sender, encrypted_text, security_key FROM messages WHERE access_key = ? AND receiver = ?");
        $stmt->bind_param("is", $accessKey, $email);
        $stmt->execute();
        $stmt->store_result();
        
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($sender, $encryptedText, $dbSecurityKey);
            $stmt->fetch();
                ...
                try {
                    ...
                    if ($senderMail) {
                        ...
                        try {
                            ...
                            $mail->SMTPAuth   = true;
                            $mail->Username   = 'jayaprathapjai10@gmail.com';
                            $mail->Password   = 'atcr ibjn xcxp mmfr';
                            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
                            $mail->Port       = 587;

                            // Recipients
                            $mail->setFrom('jayaprathapjai10@gmail.com', 'CryptChat');
